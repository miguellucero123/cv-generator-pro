<template>
  <form class="auth-form" @submit.prevent="handleSubmit">
    <h2>{{ i18n.t ? i18n.t('auth.register') : 'Registro' }}</h2>
    <div v-if="auth.error.value" class="auth-error">{{ auth.error.value }}</div>
    <div class="form-group">
      <label>Nombre</label>
      <input v-model="name" type="text" required placeholder="Tu nombre" minlength="2" />
    </div>
    <div class="form-group">
      <label>Email</label>
    ...
    <div class="form-group">
      ...
      <input v-model="password" type="password" required placeholder="Mín. 6 caracteres, 1 número" minlength="6" />
    </div>
    <button type="submit" class="btn-submit" :disabled="auth.loading.value">
      <span v-if="auth.loading.value">...</span>
      <span v-else>{{ i18n.t ? i18n.t('auth.register') : 'Registrarse' }}</span>
    </button>
    <p class="auth-link">
      ¿Ya tienes cuenta?
      <router-link to="/auth/login">{{ i18n.t ? i18n.t('auth.login') : 'Entrar' }}</router-link>
    </p>
  </form>
</template>
